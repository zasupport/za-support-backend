...
async def get_installer(
    ...
    lines = [
        ...
        '# Store bootstrap URL so the hourly updater can auto-repair settings.conf if it gets wiped',
        f'echo "{API_URL}/agent/repair?client_id={client_id}&token={token}" > "$INSTALL_DIR/config/.bootstrap_url"',
        'chmod 600 "$INSTALL_DIR/config/.bootstrap_url"',
        'echo "[OK] settings.conf + bootstrap_url"',
        "",
...
async def get_repair_script(
    ...
    lines = [
        ...
        f'echo "{API_URL}/agent/repair?client_id={client_id}&token={token}" > "$INSTALL_DIR/config/.bootstrap_url"',
        'chmod 600 "$INSTALL_DIR/config/.bootstrap_url"',
        'log "[OK] settings.conf + bootstrap_url"',
        "",

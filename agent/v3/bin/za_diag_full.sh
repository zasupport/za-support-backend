...
# Resolve project root and load client config early
PROJECT_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
source "$PROJECT_ROOT/config/settings.conf" 2>/dev/null || true
source "$PROJECT_ROOT/config/pricing.conf" 2>/dev/null || true
...
if [[ -z "${CLIENT_ID:-}" ]]; then
    CLIENT_ID=$(lookup_client_id "$SERIAL")
fi
...
if [[ -n "${ZA_VT_API_KEY:-}" || -n "${ZA_ABUSEIPDB_KEY:-}" || -n "${ZA_HIBP_API_KEY:-}" ]]; then
    source "$PROJECT_ROOT/modules/threat_intel.sh"
...
source "$PROJECT_ROOT/modules/malware_scan.sh"
...
# ═══════════════════════════════════════════════════════════════
# VERIFICATION AGENT — Second-pass QA
# ═══════════════════════════════════════════════════════════════
progress "VER/$TOTAL_SECTIONS — Verification Agent (data quality assurance)"
source "$PROJECT_ROOT/modules/verification_agent.sh"
run_verification "$JSON_FILE" "$REPORT_FILE"
...
if [[ "$PUSH_MODE" == true ]]; then
    ...
    source "$PROJECT_ROOT/modules/render_sync.sh"
